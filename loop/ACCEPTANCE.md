...
- [x] `query(source)` returns a `Query<T>`; the engine NEVER mutates the source array or its
      rows (proven by tests that snapshot and re-compare the source after execute).
- [x] `where(key, op, value)`: full operator set (`==`, `!=`, `<`, `<=`, `>`, `>=`, `in`) with
...
- [x] `where(predicate)` overload works and is typed (`(row: T) => boolean`).
- [x] `sort(key, direction?)`: stable; chained calls compose as tie-breakers (first call
      primary); `null`/`undefined` sort last regardless of direction.
- [x] `limit(n)`: applies at its pipeline position; throws `TypeError` at call time for
      negative or non-integer n; `limit(0)` yields empty.
- [x] Ops apply in CALL order (e.g. `limit` before `where` truncates first) and `explain()`
      returns serializable op descriptions in exactly that order.
- [x] `select(...keys)`: projects rows to exactly the named keys; result type is
      `Pick<T, K>`; querying a selected-away key afterwards does not compile.
- [x] `groupBy(key).execute()`: `Map<T[K], T[]>`, SameValueZero grouping, first-seen group
      order, pipeline order within groups.
- [x] `groupBy(key).aggregate(spec)` with `agg.count/sum/avg/min/max`: result rows
      `{ key } & { [name]: number }`, names and types inferred from the spec.
- [x] Ungrouped aggregates `count/sum/avg/min/max` on `Query<T>`; number-keyed only;
      empty-set semantics per DESIGN 7 (`count`/`sum` -> 0; `avg`/`min`/`max` throw
      `RangeError` naming the aggregate and key).
- [x] Branching works: two queries extended from one shared prefix are independent.

## Type-level (checked by `tsc --noEmit` on `src/type-tests.ts`)

- [x] Positive: `select` -> `Pick`, `groupBy().execute()` -> `Map<T[K], T[]>`,
      `aggregate` -> inferred named numbers, `where` value locked to `T[K]`
      (`in` -> `readonly T[K][]`) — asserted with an `Expect<Equal<...>>` helper.
- [x] Negative (each with `@ts-expect-error`): unknown key; relational op on
...
- [x] Built test-first; every new behavior has tests derived from the criteria above
- [x] Full gate green on a clean working tree: `./loop/scripts/gate.sh`
- [x] 100% line + function coverage across all `src/` runtime files (bunfig threshold)
- [x] No placeholder/stub implementations in shipped code paths
- [x] Zero runtime dependencies (`dependencies` empty in package.json)
- [x] "Readable in one sitting" verdict PASS from a fresh-context judge over `src/`
      (see `.claude/agents/judge.md`); verdict and applied findings recorded in
      `loop/PROGRESS.md`

## Documentation

- [x] `README.md` usage examples match the actual API and were executed before being written
      down (measured, not aspirational)
- [x] `docs/DESIGN.md` open questions updated for anything resolved
- [x] `loop/PROGRESS.md` and `loop/HANDOFF.md` reflect actual state

## Process

- [x] All tasks in `loop/IMPLEMENTATION_PLAN.md` for this milestone are `[x]`
- [x] Decisions made autonomously during the loop are recorded in `loop/PROGRESS.md`
